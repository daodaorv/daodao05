...
<template>
  <div class="profit-hosting-container">
    <PageHeader title="托管分润管理" description="管理托管车辆收益分润和结算" />
      ...
      <el-col :span="6">
        <el-card shadow="hover" class="stat-card">
          <div class="stat-content">
            ...
            <div class="stat-info">
              <div class="stat-label">车主总金额</div>
              <div class="stat-value" style="color: #67c23a">
                ¥{{ stats.totalOwnerAmount.toLocaleString() }}
              </div>
            </div>
          </div>
        </el-card>
    ...
    >
      ...
      <template #subsidyInfo="{ row }">
        <div style="font-size: 12px">
          <div v-if="row.guaranteedIncome > 0" style="color: #409eff">
            保底: ¥{{ row.guaranteedIncome }}
          </div>
          <div v-if="row.subsidyAmount > 0" style="color: #e6a23c">
            补贴: ¥{{ row.subsidyAmount }}
          </div>
          ...
          <el-descriptions :column="1" border>
            <el-descriptions-item label="总收益">
              <span style="color: #f56c6c; font-weight: bold; font-size: 18px">
                ¥{{ currentRecord.totalIncome.toLocaleString() }}
              </span>
            </el-descriptions-item>
            ...
            <el-descriptions-item v-if="currentRecord.guaranteedIncome > 0" label="保底收益">
              <span style="color: #409eff">¥{{ currentRecord.guaranteedIncome }}</span>
            </el-descriptions-item>
            <el-descriptions-item v-if="currentRecord.subsidyAmount > 0" label="淡季补贴">
              <span style="color: #e6a23c">¥{{ currentRecord.subsidyAmount }}</span>
            </el-descriptions-item>
            <el-descriptions-item v-if="currentRecord.ownerUsageFee > 0" label="车主自用费用">
              <span style="color: #909399">¥{{ currentRecord.ownerUsageFee }}</span>
            </el-descriptions-item>
          </el-descriptions>
        </el-card>
...
import {
  getHostingProfitList,
  getHostingProfitStats,
  settleHostingProfit,
  batchSettleHostingProfit,
  payHostingProfit,
...
// 搜索表单
const searchForm = reactive<HostingProfitListParams>({
  keyword: '',
...
const fetchStats = async () => {
  try {
    const res: any = await getHostingProfitStats()
...
// 结算
const handleSettle = async (row: HostingProfitRecord) => {
  try {
    ...
    await settleHostingProfit(row.id)
...
const handleBatchSettle = async () => {
  ...
  try {
    ...
    await batchSettleHostingProfit(selectedIds.value)
...
// 支付
const handlePay = (row: HostingProfitRecord) => {
  currentRecord.value = row
...
const handleConfirmPayment = async () => {
  ...
  try {
    await payHostingProfit(currentRecord.value!.id, paymentForm.paymentMethod)
...
const handleExport = async () => {
  try {
    const res: any = await exportHostingProfitRecords(searchForm)
